...
const handleButtonClick = (e) => {
    ...
    if (buttonId === 'Abbey'){
        bodySelector.style.background = 'lightyellow';
    } else if (buttonId === 'Doc'){
        bodySelector.style.background = 'lightgreen';
    } else if (buttonId === 'Jacob'){
        bodySelector.style.background = 'pink';
    } else {
...
const pieBuilder = (arg) => {
    let domString = '';
    
    for (let i = 0; i < arg.length; i++) {
        domString += `<div class="card my-4" style="width: 18rem;" id="${i}">
                        <div class="img-container" style="background-image: url(${arg[i].imageUrl})">
                        </div>
                        <div class="card-body">
                            <p class="card-text">${arg[i].name}</p>
                        </div>
                        <button type="button" class="btn btn-danger" id="${i}">Eat Me!</button>
                    </div>`
    }
